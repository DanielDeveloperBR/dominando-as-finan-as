...
export class AuthService {
  static async registrar(nome: string, email: string, senha: string, salarioMensal: number): Promise<Usuario> {
    ...
    const res = await query(`INSERT INTO users (nome, email, password_hash, salario_mensal) VALUES ($1, $2, $3, $4) RETURNING id, nome, email, salario_mensal as "salarioMensal", created_at`, [nome, email, password_hash, salarioMensal]);
  ...
  static async buscarPorId(id: string): Promise<Usuario | null> {
    const res = await query(`SELECT id, nome, email, salario_mensal as "salarioMensal", created_at FROM users WHERE id = $1`, [id])

    return res.rows[0] || null;
  }

  static async buscarPorEmail(email: string): Promise<{ id: string; nome: string } | null> {
    const res = await query(
      `SELECT id, nome FROM users WHERE email = $1 LIMIT 1`,
      [email]
    );
