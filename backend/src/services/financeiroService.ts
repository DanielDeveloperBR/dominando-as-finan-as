...
export class FinanceiroService {
  ...
  static async calcularResumo(userId: string) {
    const res = await query(`SELECT COALESCE(SUM(CASE WHEN tipo = 'RECEITA' THEN valor ELSE 0 END), 0) as receita,
       COALESCE(SUM(CASE WHEN tipo = 'DESPESA' THEN valor ELSE 0 END), 0) as despesa FROM transactions WHERE user_id = $1`, [userId]);
  ...
  static async getSummaryCompleto(userId: string) {
    ...
    if (salarioMensal <= 0) {
      return {
        error: 'Defina seu salário mensal para gerar o score.'
      };
    }

    const transacoesRes = await query(
      `SELECT valor, tipo, data FROM transactions WHERE user_id = $1`,
      [userId]
    );
  ...
  static async getHistoricoScore(userId: string) {
    const res = await query(
      `SELECT ano, mes, score, saldo_previsto
     FROM monthly_scores
     WHERE user_id = $1
